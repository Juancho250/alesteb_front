...
export default function SalesHistory() {
  ...
  // Acceso directo sin validación de permisos
  ...
  return (
    <div className="pb-24 bg-[#F8FAFC] min-h-screen font-sans text-slate-900">
              ...
              >
                <div className="flex items-center gap-5">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        {sale.sale_type === 'online' ? <Globe size={22} /> : <Store size={22} />}
                    </div>
                    <div>
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right">
                        ...
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                            sale.sale_type === 'online' ? 'bg-violet-100 text-violet-600' : 'bg-emerald-100 text-emerald-600'
                        }`}>
                            {sale.sale_type === 'online' ? 'Online' : 'Local'}
                        </span>
                    </div>
      ...
      {/* MODAL DETALLE - FIXED Z-INDEX */}
      {open && currentSale && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200 print:hidden">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-300 relative z-[100000]">
            
            <div className="bg-slate-50 p-6 text-center border-b border-slate-100 relative">
                <button onClick={closeModal} className="absolute top-5 right-5 p-2 bg-white hover:bg-slate-200 rounded-full text-slate-500 transition-colors shadow-sm z-10">
                    <X size={18} />
                </button>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button onClick={closeModal} className="py-3.5 rounded-xl font-bold text-sm bg-white border border-slate-200 text-slate-600 hover:bg-slate-50">
                        Cerrar
                    </button>
                    <button onClick={handlePrint} className="py-3.5 rounded-xl font-bold text-sm bg-slate-900 text-white hover:bg-black shadow-lg flex items-center justify-center gap-2">
                        <Printer size={18} />
                        Imprimir
                    </button>
                </div>
      ...
      {/* VISTA IMPRESIÓN */}
      <div className="hidden print:block print:absolute print:top-0 print:left-0 print:w-full print:bg-white z-[9999]">
         {currentSale && (
             <div className="w-[80mm] mx-auto font-mono text-black p-4">
                 <div className="text-center mb-6 border-b border-black/10 pb-4">
                     ...
                     <p className="text-[10px]">NIT: 900.123.456-7</p>
                     <p className="text-[10px]">Calle 123 #45-67, Ciudad</p>
                     <p className="text-[10px]">Fecha: {new Date().toLocaleString()}</p>
                 </div>
